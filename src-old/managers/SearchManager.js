...
class SearchManager {
  ...
  async search(query, options = {}) {
    ...
    this.client.emit('debug', `[SearchManager] Starting search: "${query}" (Source: ${options.source || 'default'})`);
    ...
    if (this.cache.has(cacheKey)) {
      ...
      if (Date.now() - cached.timestamp < this.cacheTTL) {
        this.client.emit('debug', `[SearchManager] Cache Hit: "${query}"`);
        return cached.data;
      }
    ...
    if (isURL && query.includes('youtube.com')) {
      ...
      if (list) {
        this.client.emit('debug', `[SearchManager] YouTube Playlist/Mix detected, normalizing URL...`);
        identifier = `https://www.youtube.com/playlist?list=${list}`;
      }
    }

    this.client.emit('debug', `[SearchManager] Sending to Lavalink: "${identifier}"`);

    try {
      const node = options.node ? this.client.nodes.get(options.node) : this.client.nodes.getLeastUsed();
      if (!node) throw new Error('No nodes available for search');

      let result = await node.rest.loadTracks(identifier);
      let parsedResult = this._parseSearchResult(result, options.requester);

      this.client.emit('debug', `[SearchManager] Lavalink returned: ${parsedResult.loadType} (${parsedResult.tracks.length} tracks)`);
      ...
      if ((parsedResult.loadType === 'empty' || parsedResult.loadType === 'error' || parsedResult.tracks.length === 0) && isURL) {
        this.client.emit('debug', `[SearchManager] Smart Resolve enabled for: "${query}"`);
        const resolved = await this._smartResolve(query, options.requester);
        if (resolved) {
          this.client.emit('debug', `[SearchManager] Smart Resolve success: ${resolved.tracks.length} tracks found`);
          parsedResult = resolved;
        } else {
          this.client.emit('debug', `[SearchManager] Smart Resolve failed to find results.`);
        }
      }
      ...
      this.client.emit('debug', `[SearchManager] Critical search error: ${error.message}`);
  ...
  _getSearchIdentifier(query, source) {
    ...
    const sourceEngine = source || this.client.options.searchEngine || 'youtubemusic';
    let prefix = SearchPrefix.YOUTUBE_MUSIC;

    switch (sourceEngine.toLowerCase()) {
      case 'youtube': case 'yt': prefix = SearchPrefix.YOUTUBE_MUSIC; break;
  ...
  /**
   * Autocomplete search for Discord (Advanced Mixed Search)
  ...
  async autocomplete(query, options = {}) {
    ...
    const sources = options.sources || ['youtubemusic', 'spotify', 'soundcloud'];
    const limitPerSource = Math.ceil(25 / sources.length);
    
    try {
      // Parallel search across multiple sources
      const searchPromises = sources.map(source => 
        this.search(query, { ...options, source, limit: limitPerSource })
          .catch(() => ({ tracks: [] }))
      );
      ...
      for (let i = 0; i < limitPerSource; i++) {
        for (const result of allResults) {
          if (result && result.tracks && result.tracks[i]) {
            combinedTracks.push(result.tracks[i]);
          }
        }
      }
      ...
      return combinedTracks
        ...
        .map(track => {
          ...
          // Format: [Source] Title - Author (Duration)
          const durationStr = track.duration > 0 ? ` (${formatDuration(track.duration)})` : '';
          const name = `${icon} [${sourceName}] ${track.title} - ${track.author}${durationStr}`;
      ...
      this.client.emit('debug', `Autocomplete mixed search failed: ${error.message}`);
      return [];
    }
  }
  ...
  _needsSmartResolve(url) {
    return /spotify\.com|apple\.com|deezer\.com|soundcloud\.com|youtube\.com\/playlist/.test(url);
  }

  /**
   * Universal Smart Resolve
    ...
    try {
      ...
      for (let i = 0; i < rawTracks.length; i += batchSize) {
        ...
        const resolvedChunk = await Promise.all(chunk.map(async (t) => {
          ...
          const res = await this._searchWithFallback(query, requester);
    ...
    try {
      ...
      for (let i = 0; i < trackNames.length; i += batchSize) {
        ...
        const resolvedChunk = await Promise.all(chunk.map(async (name) => {
          const res = await this._searchWithFallback(name, requester);
          return res.tracks?.[0] || null;
        }));
        tracks.push(...resolvedChunk.filter(t => t !== null));
  ...
  /**
   * Helper to search with fallback between engines
   * @private
   */
  async _searchWithFallback(query, requester) {
    this.client.emit('debug', `[SmartResolve] Searching with fallback for: "${query}"`);
    
    // Try YouTube Music first (usually better quality/metadata)
    const ytmResult = await this.search(query, { requester, source: 'youtubemusic' });
    if (ytmResult.tracks && ytmResult.tracks.length > 0) {
      this.client.emit('debug', `[SmartResolve] Success on YouTube Music for: "${query}"`);
      return ytmResult;
    }

    // Fallback to standard YouTube
    this.client.emit('debug', `[SmartResolve] YouTube Music with no results for "${query}", trying standard YouTube...`);
    const ytResult = await this.search(query, { requester, source: 'youtube' });
    
    if (ytResult.tracks && ytResult.tracks.length > 0) {
      this.client.emit('debug', `[SmartResolve] Success on standard YouTube for: "${query}"`);
    } else {
      this.client.emit('debug', `[SmartResolve] Total failure: No results found in any source for: "${query}"`);
    }
    ...
    try {
      ...
      for (let i = 0; i < trackData.length; i += batchSize) {
        ...
        const resolvedChunk = await Promise.all(chunk.map(async (track) => {
          ...
          const res = await this._searchWithFallback(`${name} ${artist}`, requester);
          return res.tracks?.[0] || null;
        }));
