...
const { YukufyClient } = require('./main');
...
const config = {
  ...
  token: process.env.BOT_TOKEN || 'YOUR_BOT_TOKEN_HERE', // Replace with your actual token
  clientId: process.env.CLIENT_ID || 'YOUR_CLIENT_ID_HERE', // Replace with your bot's application ID
...
const commands = [
  ...
  // Skip command
  {
      ...
      async execute(interaction) {
          ...
          try {
              // Player.js v1.7.0 skip method expects guildId
  ...
  // Stop command
  {
      ...
      async execute(interaction) {
           ...
           const connection = getVoiceConnection(guildId);
           if (!connection) {
               return interaction.reply({ content: 'I\'m not connected to a voice channel here.', ephemeral: true });
           }
  ...
  // Leave command
  {
      ...
      async execute(interaction) {
          ...
          const connection = getVoiceConnection(guildId);
          if (!connection) {
               return interaction.reply({ content: 'I\'m not connected to a voice channel in this server.', ephemeral: true });
          }
  ...
  // Loop command
  {
      ...
      async execute(interaction) {
          ...
          if (!voiceChannel) {
           ...
           const connection = getVoiceConnection(guildId);
           if (!connection) {
               return interaction.reply({ content: 'I need to be connected to a voice channel first.', ephemeral: true });
           }
  ...
  // Volume command
  {
      ...
      async execute(interaction) {
          ...
          if (!voiceChannel) {
           ...
           const connection = getVoiceConnection(guildId);
           if (!connection) {
               // Allow setting default volume even if not connected? Or require connection?
               // Let's require connection for clarity.
                return interaction.reply({ content: 'I need to be connected to a voice channel first.', ephemeral: true });
           }
            // Optional: Check if user is in the same channel
            if (connection.joinConfig.channelId !== voiceChannel.id) {
