...
export class LavalinkNode extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.#ws = new WebSocket(wsUrl, { headers: { Authorization: this.#password, 'User-Id': this.#client.clientId ?? this.#client.discordClient.user?.id ?? 'suwaku', 'Client-Name': 'Suwaku/1.2.5' } });
